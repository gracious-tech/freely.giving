    ...
    transition(:name="`slide-${transition_direction}`" mode="out-in")
        ...
        section.section(v-else-if="current_step === 1.1 || current_step === 2.1" key="no_funding")
            .content
                ...
                .text
                    ...
                    p Go back if you'd like to change your answer, or otherwise see this article on #[GO(to='https://sellingjesus.org/articles/paying-pastors') why pastors should be paid].
            .controls
                button.btn.back(@click="go_back") Back
        ...
        section.section(v-else-if="current_step === 5" :key="5")
            .content
                ...
                .options
                    button.option(@click="toggle_item(justifying_verses, 'luke_10_7')" :class="justifying_verses.includes('luke_10_7') ? 'selected' : ''") {{ verse_texts.luke_10_7 }}
                    button.option(@click="toggle_item(justifying_verses, '1_tim_5_18')" :class="justifying_verses.includes('1_tim_5_18') ? 'selected' : ''") {{ verse_texts['1_tim_5_18'] }}
                    button.option(@click="toggle_item(justifying_verses, '1_cor_9_14')" :class="justifying_verses.includes('1_cor_9_14') ? 'selected' : ''") {{ verse_texts['1_cor_9_14'] }}
                    button.option(@click="other_selected = !other_selected" :class="other_selected ? 'selected' : ''") Other
            .controls
                button.btn.back(@click="go_back") Back
                button.btn.cont(@click="next_step") Continue
        ...
        section.section.six(v-else-if="current_step === 6" :key="6")
            .content
                .theme-boxes
                    //- NOTE div parent important for grid ordering
                    div
                        p
                            strong
                                | Many passages affirm that it is appropriate to receive wages for ministry.
                                | {{ justifying_verses.length > 0 ? 'Such as...' : '' }}
                        div.small
                            p(v-if="justifying_verses.includes('luke_10_7')") {{ verse_texts['luke_10_7'] }}
                            p(v-if="justifying_verses.includes('1_tim_5_18')") {{ verse_texts['1_tim_5_18'] }}
                            p(v-if="justifying_verses.includes('1_cor_9_14')") {{ verse_texts['1_cor_9_14'] }}
                    p
                        strong But do they suggest such wages can be obtained by charging for the ministry?
                    img(src='../assets/tract_plant.jpg')
                    img(src='../assets/tract_gift.jpg')
            ...
            .content
                ...
                .table-section
                    ...
                    .table-item
                        ...
                        p He permitted them to receive hospitality, but forbade them from expecting it from every person they served.
                        p.verse-ref (Luke 10:7)
                    .table-item
                        strong but through donations, not sales.
...
const other_selected = ref(false)
...
// Verse texts
const verse_texts = {
    'luke_10_7': '“The worker is worthy of his wages.” (Luke\u00A010:7)',
    '1_tim_5_18': '“Do not muzzle an ox while it is treading out the grain.” (1\u00A0Tim\u00A05:18)',
    '1_cor_9_14': '“those who preach the gospel should receive their living from the gospel.” (1\u00A0Cor\u00A09:14)'
}
